...
public class TestVitalSigns extends Activity {
	private TestVitalSigns _activity;
    ...
    double heartRate = 0;
    int frameNumber = 1;
	int left = 0, top = 0, right = 0, bottom = 0, smallPreviewWidth = 0, smallPreviewHeight = 0, numberOfPixelsToAnalyze = 0;
    ...
    public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
        ...
        public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        	...
        	previewTop    = (int) (previewHeight * .25);
        	previewRight  = (int) (previewWidth  * .70);
        	previewBottom = (int) (previewHeight * .40);
            ...
            try {
    		    ...
    		    ImageView mImageView = new ImageView(_activity);
...
//    		    mImageViewRectangle0.setLayoutParams(new RelativeLayout.LayoutParams(rectangleWidth, rectangleHeight));
//    		    mImageViewRectangle0.setPadding(previewLeft, previewTop, previewRight, previewBottom);
...
//	        				mImageViewRectangle0.setPadding(previewLeft, previewTop, previewRight, previewBottom);
//	        				mImageViewRectangle0.setMaxHeight(previewRight-previewLeft);
//	        				mImageViewRectangle0.setMaxWidth(previewBottom-previewTop);
//	        				mImageViewRectangle0.setMinimumHeight(previewRight-previewLeft);
//	        				mImageViewRectangle0.setMinimumWidth(previewBottom-previewTop);
//		    		    	mImageViewRectangle0.invalidate(); // Was postInvalidate, but that is for non-UI threads
	        			}
	        			frameNumber = 0;	    		    	
	        		} else {
	        			if (frameNumber == 0) {
	        				left = previewLeft; top = previewTop; right = previewRight; bottom = previewBottom;
	        				smallPreviewWidth = right - left+1; // because coordinate system is different and backwards // 731
		                	smallPreviewHeight = bottom - top+1; // 1300
...
//			        	Log.d("DEBUG", "DEBUG: Bitmap Width,Height = " + smallPreviewWidth + "," + smallPreviewHeight);
				        ...
				        if(frameNumber < heartRateFrameLength) {
				        	...
				        	// TODO: show calculating instead of 1.. and 2.. right before doing ICA and FFT
    ...
    class MyFaceDetectionListener implements Camera.FaceDetectionListener {
	    public void onFaceDetection(Face[] faces, Camera camera) {
	        ...
	        if (numberOfFacesCurrentlyDetected > 0) {
		    	...
		    	mImageViewRectangle0.setPadding(previewLeft, previewTop, previewWidth-previewRight, previewHeight-previewBottom);  // TODO: Add back after taking pictures without rectangle // TODO: change coordinate system for left and top
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.menu_settings: // TODO: Change to Reset Default, or set default.
            	// Basically do nothing, except reset to defaults
            ...
            case R.id.menu_sendSMS:
                ...
                return true;
            case R.id.menu_cycleCamera: // TODO: Fix this. It doesn't work.
                // check for availability of multiple cameras
            	int numberOfCameras = Camera.getNumberOfCameras();
                if (numberOfCameras == 1) {
                	Toast.makeText(_activity, "No other cameras available", Toast.LENGTH_LONG).show();
                    return true;
                }

                // OK, we have multiple cameras. Which one do we currently have? And set defaultCameraId to the camera we want next
                CameraInfo cameraInfo = new CameraInfo();
                android.hardware.Camera.getCameraInfo(defaultCameraId, cameraInfo);
                ...
                mCamera = getCameraInstance(defaultCameraId);
                mFrameLayoutCameraPreview.addView(new CameraPreview(_activity, mCamera)); // Create and add camera preview to screen
