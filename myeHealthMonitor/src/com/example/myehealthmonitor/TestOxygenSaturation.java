...
public class TestOxygenSaturation extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        MediaPlayer mp = MediaPlayer.create(this, R.raw.oxysat);
		mp.start();
    ...
    public void calculateO2(double r, double nearInfrared) { // TODO: change red to array
		...
		if(config == 1) {
			...
			result = o2_level(red, nir);
			Toast.makeText(_activity, "result: " + result , Toast.LENGTH_SHORT).show();
			SQLiteDatabase db = openOrCreateDatabase("MyDB", MODE_PRIVATE,  null);
			db.execSQL("CREATE TABLE IF NOT EXISTS PatientData (Username VARHCAR, DataType VARCHAR, Result VARCHAR);");
			db.execSQL("INSERT INTO PatientData VALUES('doc222', 'Oxygen Saturation', result);");
			db.close();
		}
	}
	
	public double o2_level(double hbO2, double hb) {
		Toast.makeText(_activity, "result: " + hbO2/(hbO2+hb), Toast.LENGTH_SHORT).show();
		return hbO2/(hbO2+hb);
	}
